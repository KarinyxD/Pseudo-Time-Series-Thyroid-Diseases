...
def preprocessing_pts():
    ...
    # 2. Limpeza Básica de Target
    df = df.dropna(subset=['target'])
    df['target'] = df['target'].str.strip().str.upper()
    
    # 3. FILTRAGEM: Manter apenas Saudáveis e Hipo (Remove Hiper)
    pattern = '|'.join(classes)
    df = df[df['target'].str.contains(pattern, na=False, regex=True)]
    
    # 4. Limpeza de Labels (Pega a primeira letra relevante)
    # Ex: "F123" vira "F"
    def clean_target(x):
    ...
    # Mapear para números ordinais para validação posterior
    df['severity_label'] = df['target_clean'].map(class_mapping_details)
    
    # 5. Aplicação dos Limites (Outliers)
    ...
    # 8. PADRONIZAÇÃO (Z-SCORE) - Passo Vital para Distância Euclidiana 
    # TSH e T4 na mesma escala de importância
    ...
    # Retornamos:
    # 1. df_normalized: Para calcular a matriz de distância (Matemática)
    # 2. df_imputed: Os valores reais para plotar gráficos depois (Interpretação)
    # 3. df['severity_label']: As classes originais para validar
